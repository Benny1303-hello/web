...
import { systemIntegrationStaff } from '@/lib/content';
...
export default function StaffContent() {
  ...
  return (
    <>
      <PageHero
        crumb={t('pages.systemIntegrationStaff.hero.crumb')}
        eyebrow={t('pages.systemIntegrationStaff.hero.eyebrow')}
        title={t('pages.systemIntegrationStaff.hero.title')}
        description={t('pages.systemIntegrationStaff.hero.description')}
      />

      <section className="bg-mist-50 py-20">
        <div className="container-page">
          <Reveal className="mx-auto max-w-2xl text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-600">{t('pages.systemIntegrationStaff.contactEyebrow')}</p>
            <h2 className="mt-3 text-balance font-display text-3xl font-bold text-navy-900">
              {t('pages.systemIntegrationStaff.contactHeading')}
            </h2>
          </Reveal>

          <div className="mx-auto mt-12 max-w-4xl space-y-12">
            {systemIntegrationStaff.map((group) => (
              <div key={group.groupKey}>
                <h3 className="mb-5 font-display text-lg font-bold text-navy-900">
                  {t(`pages.systemIntegrationStaff.staffGroups.${group.groupKey}`)}
                </h3>
                    ...
                    <Reveal key={contact.key} delay={i * 0.06}>
                      <div className="h-full rounded-xl bg-white px-6 py-5 shadow-soft ring-1 ring-black/5">
                        ...
                        <p className="mt-1 text-xs font-medium uppercase tracking-wide text-brand-600">
                          {t(`pages.systemIntegrationStaff.staffRoles.${contact.key}`)}
                        </p>
